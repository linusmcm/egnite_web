...
ui <- fullPage(
     ...
     , HTML('<script>
       $( document ).on("shiny:sessioninitialized", function(event) {
           Shiny.setInputValue("visShinyFocus", ""id":"intro","focusId":["egnite"],"options":{"scale":1,"offset":{"x":-450,"y":-100},"locked":true,"animation":{"duration":500,"easingFunction":"easeInOutQuad"}}}");
       });</script>')
    ...
    , fullSection(menu = "egnite"
                    , absolutePanel(tags$a(href='.http://egnite.com.au',target="_blank", tags$img(src='logo_blue_flame_web.png', width='50%')), bottom = '5px', right = '5px')
    ...
    , fullSection(menu = "partners"
                  , absolutePanel(tags$a(href='.http://egnite.com.au',target="_blank", tags$img(src='logo_blue_flame_web.png', width='50%')), bottom = '5px')
                  , h1("egnite partners")
...
server <- function(input, output)
{
    ...
    output$intro <- renderVisNetwork(
    {
        visNetwork(NODES, edges, width = "100%", height = "100%") |>
           ...
           #visPhysics(solver = "barnesHut", maxVelocity = 1, barnesHut= list(avoidOverlap = 1, damping = 1)) |>
